...
export type NodeVersionSource = "package.json engines" | ".nvmrc" | ".node-version" | "runtime";

export interface DetectedNodeVersion {
  version: string;
  source: NodeVersionSource;
}

export function detectNodeVersionWithSource(files: { path: string; content: string }[]): DetectedNodeVersion {
  const meta = extractPackageMeta(files);
  if (meta.engines?.node) {
    const v = meta.engines.node.replace(/[^0-9.]/g, "").split(".")[0];
    if (v) return { version: v, source: "package.json engines" };
  }

  const nvmrc = files.find((f) => f.path === ".nvmrc" || f.path.endsWith("/.nvmrc"));
  if (nvmrc) {
    const v = nvmrc.content.trim().replace(/^v/, "").split(".")[0];
    if (v && /^\d+$/.test(v)) return { version: v, source: ".nvmrc" };
  }

  const nodeVersionFile = files.find((f) => f.path === ".node-version" || f.path.endsWith("/.node-version"));
  if (nodeVersionFile) {
    const v = nodeVersionFile.content.trim().replace(/^v/, "").split(".")[0];
    if (v && /^\d+$/.test(v)) return { version: v, source: ".node-version" };
  }

  return { version: process.versions.node.split(".")[0], source: "runtime" };
}

export function detectNodeVersion(files: { path: string; content: string }[]): string {
  return detectNodeVersionWithSource(files).version;
}
...
function generateCiWorkflow(meta: PackageMeta, nodeVersion: string): string {
  const scripts = meta.scripts ?? {};
...
router.post("/github/push", async (req, res) => {
  try {
    ...
    const { version: nodeVersion, source: nodeVersionSource } = detectNodeVersionWithSource(typedFiles);
    ...
    if (genCi) {
      const ciContent = generateCiWorkflow(pkgMeta, nodeVersion);
    ...
    logger.info({ repoUrl, committed, commitSha, isNewRepo, filesAdded, filesChanged, nodeVersion, nodeVersionSource }, "GitHub push complete");
    res.json({
      success: true,
      repoUrl,
